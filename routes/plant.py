...
from db.utils.plant import create_plant, edit_plant, get_plant_by_id, delete_plant_by_id, get_plants
...
@plant_router.get('/get-plants', response_model=List[PLantResponse])
async def get_plants_path(page: int = 1, per_page: int = 20, session: AsyncSession = Depends(get_session)):
    return await get_plants(page, per_page, session)
