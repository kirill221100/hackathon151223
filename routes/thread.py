...
from db.utils.thread import create_thread, get_thread_by_id, get_thread_by_id_with_messages, edit_thread, \
    delete_thread_by_id, get_threads
...
@thread_router.get('/get-threads', response_model=List[ThreadResponse])
async def get_threads_path(page: int = 1, per_page: int = 10, session: AsyncSession = Depends(get_session)):
    return await get_threads(page, per_page, session)
